...
class UnitEconomicsCacheController extends Controller
{
    ...
    private function buildUnitEconomicsSpreadsheet(
        ...
        $avgCols = array_values(array_intersect(
            $isWildberriesExport
                ? ['D', 'G', 'H', 'I', 'J', 'L', 'M', 'P', 'U', 'W', 'Y', 'AA', 'AC', 'AE', 'AH', 'AI']
    ...
    private function wildberriesExportColumns(): array
    {
        ...
        return [
            ...
            // При изменении набора/порядка колонок сдвигать буквы в writeWildberriesExportRow,
            // sumCols/avgCols и profitCol/marginCol.
            'T'  => ['header' => 'Приёмка, ₽',         'width' => 11, 'field' => 'acceptance_cost',           'format' => $money],
    ...
    public function calculate(CalculateRequest $request, string $marketplace): JsonResponse
    {
        ...
        if ($marketplace === 'wildberries') {
            return response()->json([
                'message' => 'WB live calculation endpoint is deprecated. Use GET /unit-economics/wildberries for cached data (recalculated automatically after sync).',
                'data' => [
