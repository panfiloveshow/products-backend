 ...
 *
 * Зачем: отчёт остатков (POST /api/analytics/v1/stocks-report/wb-warehouses)
 * лимитирован 3 запроса/мин на токен. Внутри полного синка лимит часто уже
 * сожжён (sales/orders, локализация ИЛ/ИРП), отчёт молча пустеет, и склады
 * товара деградируют до FBS-«Мой склад» — КС юнит-экономики теряет разбивку
 * по складам (поле = средний КС магазина у всех товаров, тултип без реальных
 * складов). Здесь — отдельный запрос, затем пересчёт кэша юнит-экономики.
 *
 * Метод недоступен по обычному Базовому токену без секрета (только
 * Персональный/Сервисный/Базовый с секретом) — для таких интеграций отчёт
 * будет пустым независимо от прав категории «Аналитика».
 */
class RefreshWbStockWarehouses extends Command
{
    ...
    private function refreshIntegration(Integration $integration): void
    {
        ...
        // Ретраи на 429 (3 попытки по 20с) уже внутри WildberriesClient::analyticsPost.
        $rows = $inventory->getWbWarehousesStocksReport($integration);
