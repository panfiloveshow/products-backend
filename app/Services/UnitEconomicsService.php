...
class UnitEconomicsService
{
    ...
    private function calculateWB(array $data): array
    {
        ...
        // ИЛ с 15.08.2026 действует только переходно на FBW-остатках из поставок
        // до 15.08 (до конца 60/90-дневной фиксации) — применяем только на
        // складских схемах; на FBS не действовал и раньше (заказы-исключения).
        // ponytail: выпилить после ~15.11.2026.
        $localizationIndex = in_array($fulfillmentType, ['FBO', 'FBW'], true)
            ? (float) ($data['localization_index'] ?? 1.0)
            : 1.0;
        ...
        // Формула WB с 15.08.2026: base × КС (единый 170% / 100% СГТ) × ИЛ
        // (переходно, только FBO/FBW). ИРП отключён с 13.07.2026 (данные несут 0).
