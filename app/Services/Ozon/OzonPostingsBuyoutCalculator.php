 ...
 *
 * Формула:
 *   rate = delivered / (delivered + cancelled) * 100
 *
...
class OzonPostingsBuyoutCalculator
{
    ...
    public function calculateForSku(int $integrationId, string $sku, int $days = 28): ?array
    {
        ...
        $rows = DB::table('postings')
            ...
            ->where('postings.in_process_at', '>=', $since)
    ...
    public function calculateForIntegration(int $integrationId, int $days = 28): array
    {
        ...
        foreach ($rows as $row) {
            $sku = $row->sku;
            if (! isset($perSku[$sku])) {
                $perSku[$sku] = ['delivered' => 0, 'cancelled' => 0, 'in_flight' => 0, 'postings' => 0];
            }
            $qty = (int) $row->qty;
        ...
        foreach ($perSku as $sku => $buckets) {
            $finalized = $buckets['delivered'] + $buckets['cancelled'];
            if ($finalized === 0) {
                continue;
            }
