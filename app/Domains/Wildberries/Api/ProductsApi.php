...
class ProductsApi implements ProductsApiInterface
{
    ...
    public function getCardRatings(array $nmIds = []): array
    {
        ...
        // Период: последние 7 дней (selectedPeriod должен быть ПОСЛЕ pastPeriod)
        $selectedStart = now()->subDays(7)->format('Y-m-d');
        $selectedEnd = now()->format('Y-m-d');
        $pastStart = now()->subDays(14)->format('Y-m-d');
        $pastEnd = now()->subDays(8)->format('Y-m-d');
