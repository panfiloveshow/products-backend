...
class OzonUnitEconomicsCalculator implements UnitEconomicsCalculatorInterface
{
    ...
    private function calculateRfbs(CalculationInput $input): UnitEconomicsResult
    {
        ...
        // Невыкуп при своей доставке — оба плеча: доставка «туда» + свой возврат.
        $expectedReturnCost = ($ownDeliveryCost + $ownReturnCost) * $returnRate;
    ...
    private function calculateExpectedReturnCosts(string $scheme, float $baseLogistics, ?float $redemptionRate, ?CalculationInput $input = null): array
    {
        ...
        // Невыкупленная поездка стоит два плеча: прямую магистраль Ozon за невыкуп
        // тоже списывает (она не возвращается), плюс обратная магистраль и обработка.
        // Оба плеча приближаем базовой магистралью; последней мили в возврате нет —
        // вручения не было.
        return [
            'expected' => ($baseLogistics + $returnLogistics + $returnProcessing) * $totalReturnFraction,
    ...
    /**
     * Потери на единицу выкупа — общая математика в ReturnEconomics, метод
     * оставлен как алиас (его зовут кэш-контроллер и тесты).
     */
    public static function returnFractionPerSoldUnit(float $lostFraction): float
    {
        return \App\Domains\UnitEconomics\ReturnEconomics::fractionPerSoldUnit($lostFraction);
    }
